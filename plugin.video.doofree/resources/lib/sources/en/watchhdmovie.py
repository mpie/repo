...
class source:
    ...
    def movie(self, imdb, title, localtitle, aliases, year):
        try:
            ...
            log_utils.log('watchhdmovie - Exception: \n' + str(failure))
            return
            ...
            for link, name in Links:
                if title.lower() in name.lower():
                    if year in name:
                        ...
                        Alterjnates = re.compile('<button class="text-capitalize dropdown-item" value="(.+?)"',
                        ...
                        for alt_link in Alterjnates:
                            ...
                            if not self.filter_host(host):
                                continue
                            sources.append(
                                {'source': host, 'quality': 'HD', 'language': 'en', 'url': alt_url, 'info': [],
    ...
    def filter_host(self, host):
        if host not in ['openload.co', 'streamango.com', 'rapidvideo.com']:
            return False
        return True
